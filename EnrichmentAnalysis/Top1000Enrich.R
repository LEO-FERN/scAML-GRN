...
# Load your new combined gene file
top_genes <- read.csv("~/EnrichmentData/Combined_genes.csv", stringsAsFactors = FALSE)

# Define gene sets
gene_sets <- list(
  "H"  = msigdbr(species = "Homo sapiens", collection = "H"), #Hallmark
  ...
  "C6" = msigdbr(species = "Homo sapiens", collection = "C6") #Cancer
)

# Choose your gene set (change as needed)
selected_geneset_name <- "C6"  # Options: "Hallmark", "GO_BP", "Canonical", "Cancer"
...
results <- top_genes %>%
  ...
  group_by(CellType) %>%
  group_modify(~ {
    genes <- unlist(.x$Genes)
    genes <- trimws(genes)  # Remove whitespace if needed
...
# Save results
write.csv(final_output, paste0("~/EnrichmentData/1000_pathway_matrix_", selected_geneset_name, ".csv"), row.names = FALSE)
